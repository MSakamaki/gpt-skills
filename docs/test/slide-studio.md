...
[docs/spec/slide-studio.md](../spec/slide-studio.md) §17 受入基準 (AC-01〜37) の評価結果。
...
評価日: 2026-09-19 / 対象: `src/slide-studio/` v1.4.0 (`dist/slide-studio/skill.zip` sha256 `0f0e8ac3086e4ab29e8b74c56a0813c540e0153e7dcf6e524417a69d06248062`、79 ファイル、展開時 483.3KB、ZIP 228.1KB)

v1.0.0 の評価は同日に行い、AC-01〜24 が PASS だった。その後の実地検証で 3 件の問題が見つかり、v1.1.0 で是正した。ターン末の案内はさらに v1.2.0 で選択式へ、v1.3.0 で確認ターンを加える形へ変えた。v1.4.0 で未回答の表し方を直し、`SKILL.md` の一部を条件付き読み込みへ移した (下の「実地検証で見つかった問題」)。本書は v1.4.0 に対する再評価である。
...
| AC-30 記号の解釈が安定している | PASS | `SKILL.md`「回答の解釈」の表 (`A`/`a`/`Ａ`、`9`、`0`、自由入力、提示が無いときの裸の記号) +「実行前に何を選んだと解釈したかを 1 行返す」+「ユーザー操作の解釈」表の記号 3 行 + `SAMPLES.md` §6 |
...
| AC-32 推論で埋めず確認する | PASS | `SKILL.md` I-17 +「確認ターン」の確認する / しない の表 + Router の手順 6 + 曖昧さの多い 6 つの Context の「確認しうる論点」 |
| AC-33 確認ターンは YAML を出さない | PASS | `SKILL.md`「ターンの 2 つの形」の表と「確認の途中で現在の状況を YAML で示さない」+ 禁止事項 + `SAMPLES.md` §2 |
| AC-34 確認と判定を混ぜない | PASS | `SKILL.md`「確認ターンを出せるのは Specialist だけ」+ 禁止事項「Reviewer と Validator が判定の内容をユーザーへ聞くこと」+ Router の手順 6 |
| AC-35 委任は明示されたときだけ、記録して行う | PASS | `TURN-FORMATS.md` §1.2「まとめて委任されたとき」§1.3「記録」+ `SKILL.md`「確認ターン」の「価値判断にあたる論点には推奨も委任の選択肢も置かない」+ 結果ブロックの `clarifications` |
| AC-36 未回答を「なし」と書かない | PASS | `SKILL.md`「値の 3 つの状態」+ I-18 + 禁止事項 + `brief-normalizer` / `audience-analyzer` / `delivery-artifact-planner` のスキーマを `<未回答>` `<未取得>` へ書き換え。プレースホルダの残存を検索して 0 件 |
| AC-37 未回答を制約なしと読み替えない | PASS | `SKILL.md`「未回答は `<未回答>` のまま下流へ渡す。下流はこれを「制約が無い」と読み替えてはならない」+ `brief-normalizer` 手順 1a「文字列を書く Context が必要になった時点で確認する」+ `SAMPLES.md` §2「未回答をそのまま渡す」 |

37 件すべて PASS。**spec の要求に対して配布物へ加えるべき不足は見つからなかった。** 読み合わせで見つけた Registry と Context ファイルの不整合 (Reviewer が requires 外で読んでいた Artifact、`sync_points` の id、項目名の対応) は spec §13.7 / §13.8 へ記録したうえで是正済み。

---

## 実地検証で見つかった問題

ChatGPT 上で走らせて見つかった 4 件と、その是正を読み直して見つけた 2 件。いずれも spec を先に更新してから実装へ反映した。F-01〜F-03 は v1.1.0、F-04 は v1.2.0、F-05 は v1.3.0、F-06 は v1.4.0 で是正している。
...
| F-05 | 工程が上流に書かれていない事項を推論で埋め、根拠の無い記述が成果物に入った | 「推測で埋めない」は各 Context にあったが、埋める代わりに何をするかが決まっておらず、`open_questions` へ落として先へ進んでいた | DP-09 / I-17 を追加 (v1.3.0)。推論で埋めるしかない点は確認ターンで選択式に聞く。ターンを確認ターンと完了ターンに分け、確認ターンでは YAML を出さない。Reviewer と Validator は確認せず判定する。価値判断には推奨と委任を置かない (spec §7.1 / §13.15) |

| F-06 | 確認ターンを入れた後も、「なし」「指定なし」「未指定」として推論で埋めていた | スキーマ自身が未回答を既定値で埋めるよう指示していた。`script: なし`、`tone: 無ければ「指定なし」`、`size: null`、`status: unknown` が、確認していないことを確定した値の形で書かせていた | DP-10 / I-18 を追加 (v1.4.0)。値の状態を確定・委任・未回答の 3 つに分け、未回答は `<未回答>` と書く。下流は「制約が無い」と読み替えず、必要になった Context が確認する。該当する 3 つの Context のスキーマを書き換えた (spec §13.16) |

F-01・F-02・F-04・F-05・F-06 は spec §6 の不変条件と §7 の出力形式に関わるため、実装だけの変更ではなく仕様変更として扱った。

---

## 静的検査の結果 (test-cases.md S01〜S14)
...
| S14 値の状態 | PASS | `SKILL.md`「値の 3 つの状態」+ Context のスキーマを `: なし` `: 指定なし` `: unknown` `stated_by_user` で検索して 0 件 |
| S13 確認ターンの記述 | PASS | `SKILL.md`「ターンの 2 つの形」「確認ターン」に、境界・YAML を出さない規定・Reviewer は確認しない規定・価値判断へ推奨を付けない規定が揃っている |
...
- [x] 確認ターンと完了ターンの区別が残り、確認ターンで YAML を出さない規定がある
- [x] 推論で埋めない規定 (I-17) と、確認する / しない の境界が残っている
- [x] Reviewer と Validator が確認ターンを出さない規定が残っている
- [x] 価値判断の論点に推奨と委任の選択肢を置かない規定が残っている
- [x] 値の 3 つの状態が `SKILL.md` にあり、スキーマに「なし」「指定なし」「unknown」が既定値として残っていない
- [x] `SKILL.md` から `TURN-FORMATS.md` への委譲が保たれ、Router の手順 5 が読む条件を示している
...
| 同 Q01〜Q18 (確認ターンと値の状態) | **未実施** | F-05 と F-06 を再現する入力 (何の承認かを書かない依頼、トーンの指定が無い依頼) で、推論で埋めずに確認するか、未回答を「なし」と書かないか、確認ターンで YAML が出ないかを確認する |
| Progressive Loading | **未実施** | `SKILL.md` から `REGISTRY.md` と Context ファイル 1 本だけが読み込まれるか。毎 Turn の読込量は `SKILL.md` 30.2KB + `REGISTRY.md` 22.3KB + Context 2.4〜6.4KB + ガイドの指定節 |
...
- `SKILL.md` (30.2KB) と `REGISTRY.md` (22.3KB) は毎 Turn 読む。v1.3.0 で 32.8KB まで増えたが、v1.4.0 で条件付きの記述を `TURN-FORMATS.md` (8.9KB) へ移して 30.2KB に戻した。検証の WARN は 40KB。**次に規定を足すときも、毎ターン要るかを先に判断して委譲先を決める**
- `SAMPLES.md` は実行時に読まない。読み込ませる規定を足さない
