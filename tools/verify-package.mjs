/**
 * 生成済み dist/skill.zip を検証する。src/ ではなく配布物そのものを対象にする。
 *   node tools/verify-package.mjs
 *
 * 検査内容
 *   - ZIP 内の全ファイルが adversarial-answer/ 配下にある
 *   - 開発用ファイル (tools / docs / plans / node_modules) が混入していない
 *   - 必須ファイルが揃っている
 *   - SKILL.md の frontmatter が読める
...
import { DIST, PACKAGE_PREFIX, fmtBytes, parseFrontmatter, parseManifest, Report } from './lib.mjs';
...
const zipPath = join(DIST, 'skill.zip');
if (!existsSync(zipPath)) {
  console.error('dist/skill.zip がありません。先に npm run build を実行してください');
  process.exit(1);
}
...
for (const entry of entries) {
  if (!entry.name.startsWith(`${PACKAGE_PREFIX}/`)) {
    r.error(`トップレベルが ${PACKAGE_PREFIX}/ でない: ${entry.name}`);
    continue;
  }
  if (FORBIDDEN.test(entry.name)) r.error(`開発用ファイルが混入: ${entry.name}`);
  contents.set(entry.name.slice(PACKAGE_PREFIX.length + 1), await entry.async('nodebuffer'));
}
