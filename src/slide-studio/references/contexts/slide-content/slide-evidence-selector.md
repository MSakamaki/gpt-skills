...
| 論点 | なぜ埋め方で変わるか |
|---|---|
| 元資料に複数の数値があり、どれが主張を支えるか分からない | 選ぶ数値で図表の設計が変わる。誤った数値は下流すべてに及ぶ |
| 条件・誤差・サンプル数を残すか | 聴衆の専門性で判断が分かれるとき。情報密度と図表の複雑さが変わる |
| 出典をどこまで画面に出すか | 配布物と画面の分担が変わる |

**数値そのものを推論で埋めることは、どの場合もしない。** 元資料に無ければ `gaps` に書き、`required_evidence` に当たるなら `BLOCKED` とする。確認ターンは「元資料のどれを使うか」を聞くものであり、「無い数値を作ってよいか」を聞くものではない。
