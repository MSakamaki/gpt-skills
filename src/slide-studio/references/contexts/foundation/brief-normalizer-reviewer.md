...
| 4 | ユーザーが答えた「なし」と、確認していない `<未回答>` を区別しているか | I-18 |
...
| 9 | 依頼にトーン・表記・読解水準・言語の指定があるなら `deliverable_voice` に記録されているか。依頼に無い項目が `<未回答>` になっているか | 出力仕様 |
| 11 | **未回答を「なし」「指定なし」「未指定」と書いていないか。** ユーザーが明示的にそう答えた項目だけ「なし」と書けている。`<未回答>` の項目が `open_questions` に列挙されているか | I-18 |
| 10 | **本 Artifact 自身が作業言語 (日本語の常体、識別子は英語) で書かれているか。** 成果物向けの表記指定 (ひらがな、英語など) が記述へ適用されていないか | I-15 |

## 判定

- 観点 1・2・10・11 に該当があれば `CRITICAL`。観点 3〜7・9 は `MAJOR`。観点 8 は `MINOR`
