...
| 9 | 依頼にトーン・表記・読解水準・言語の指定があるなら `deliverable_voice` に記録されているか。`stated_by_user` が正直か | 出力仕様 |
| 10 | **本 Artifact 自身が作業言語 (日本語の常体、識別子は英語) で書かれているか。** 成果物向けの表記指定 (ひらがな、英語など) が記述へ適用されていないか | I-15 |

## 判定

- 観点 1・2・10 に該当があれば `CRITICAL`。観点 3〜7・9 は `MAJOR`。観点 8 は `MINOR`
