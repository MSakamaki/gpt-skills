...
推論で埋めるしかなく、埋め方で成果物が実質的に変わる点は、確認ターンで聞く (I-17)。この Context では次が典型となる。すべてを毎回聞くチェックリストにしない。依頼に書かれている事項は聞かない。

| 論点 | なぜ埋め方で変わるか |
|---|---|
| 聴衆に何を判断・行動してほしいか | 目的が「報告する」としか書かれていないとき。構成の骨格と成功条件が変わる |
| `use_case` が複数に当たる | 意思決定と社内報告のどちらかで、進行と情報密度の初期値が変わる |
| 依頼者と聴衆が別か | 対象者が「役員」とだけ書かれているとき。聴衆分析の前提が変わる |
| 元資料のどこまでを使ってよいか | 素材が多いとき。証拠の選定の範囲が変わる |

目的・対象者の価値判断にあたる論点には推奨を付けない。`use_case` の分類のように上流から既定を導ける論点は推奨を付けてよい。
