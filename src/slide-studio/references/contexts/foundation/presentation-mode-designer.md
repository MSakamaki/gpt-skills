...
| 論点 | なぜ埋め方で変わるか |
|---|---|
| 進行形式が複数に当たる | Workshop と登壇では、活動の有無と情報密度が大きく変わる |
| 参加・想起の機会を入れるか | 時間配分と Slide の構成が変わる。聴衆の人数と場の性質に依存する |
| author-driven / participant-driven の配分 | 構成の作り方が変わる。本人の進行の好みが要る価値判断 |
| 事前・事後の単独閲覧があるか | 配布形態の要否が変わる。依頼に書かれていないことが多い |

進行の好みは価値判断なので推奨を付けない。時間配分のように制約から導ける論点は推奨を付けてよい。
