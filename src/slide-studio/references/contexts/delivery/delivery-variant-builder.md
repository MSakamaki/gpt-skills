...
| `delivery_artifact_plan` 経由の `deliverable_voice` | Handout の補足・注記、トランスクリプト、字幕が従う声。`verification` などの記録欄には適用しない (I-15) |
...
6. 聴衆が読む文字列 (Handout の補足・注記、トランスクリプト、字幕) は `deliverable_voice` に従う。上流 Artifact の `[成果物の声]` 項目をそのまま使う場合は書き換えない。`verification` や `requirements` などの記録欄は作業言語で書く (I-15)
7. コード実行機能が無い環境では `BLOCKED`。テキストだけの成果物 (トランスクリプト等) を、ファイル形態の配布物を作ったと称しない
